...
func GetAvailable(preferred string) (Agent, error) {
	...
	// Fallback order: codex, claude-code, gemini, copilot, opencode, ollama, cursor, droid
	fallbacks := []string{"codex", "claude-code", "gemini", "copilot", "opencode", "ollama", "cursor", "droid"}
	...
	if len(available) == 0 {
		return nil, fmt.Errorf("no agents available (install one of: codex, claude-code, gemini, copilot, opencode, ollama, cursor, droid)\nYou may need to run 'roborev daemon restart' from a shell that has access to your agents")
	}
